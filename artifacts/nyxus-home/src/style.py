...
""")
    ...
    # ── OBSIDIAN REACTOR redesign (rev r2) ──────────────────────────
    for name in ("pink", "cyan", "purple", "gold", "green",
                 "orange", "red", "blue", "mono"):
        c = PALETTE.get(name, PALETTE["mono"])
        css += f"""
.ghost-glyph-{name} {{
    color: {c};
    font-size: 15px;
    text-shadow: 0 0 10px {c}, 0 0 20px alpha({c}, 0.55);
}}
.ghost-title-{name} {{
    ...
    text-shadow: 0 0 10px alpha({c}, 0.55);
}}
.ghost-rule-{name} {{
    background: linear-gradient(to right, {c},
                alpha({c}, 0.28) 40%, transparent);
    min-height: 1px;
}}
.ghost-footer-{name} {{
    font-family: "JetBrains Mono", monospace;
...
"""
    css += f"""
.ghost-card {{
    background: linear-gradient(160deg, rgba(10, 8, 22, 0.60),
                rgba(4, 3, 12, 0.42));
...
.hero-time {{
    ...
    color: {PALETTE['text']};
    letter-spacing: 0.04em;
    text-shadow: 0 0 18px alpha({PALETTE['pink']}, 0.85),
                 0 0 44px alpha({PALETTE['pink']}, 0.45),
                 0 0 80px alpha({PALETTE['pink']}, 0.25);
}}
.hero-secs {{
    font-family: "JetBrains Mono", monospace;
    font-size: 26px;
    color: {PALETTE['cyan']};
    text-shadow: 0 0 12px alpha({PALETTE['cyan']}, 0.75);
}}
.hero-date {{
    font-family: "Inter Display", sans-serif;
    font-size: 17px;
...
.hero-host {{
    ...
    color: {PALETTE['gold']};
    letter-spacing: 0.16em;
    text-shadow: 0 0 8px alpha({PALETTE['gold']}, 0.5);
}}
.hero-meta {{
    font-family: "JetBrains Mono", monospace;
    font-size: 9px;
...
.sent-count {{
    ...
    font-weight: 800;
    color: {PALETTE['text']};
    text-shadow: 0 0 10px alpha({PALETTE['text']}, 0.35);
}}
.sent-flag {{
    font-family: "JetBrains Mono", monospace;
    font-size: 10px;
...
.deck-title {{
    ...
    color: {PALETTE['text']};
    letter-spacing: 0.05em;
    text-shadow: 0 0 12px alpha({PALETTE['purple']}, 0.55);
}}
.deck-artist {{
    font-family: "JetBrains Mono", monospace;
    font-size: 10px;
...
.deck-btn {{
    background: rgba(0, 0, 0, 0.35);
    border: 1px solid alpha({PALETTE['purple']}, 0.45);
    border-radius: 6px;
...
.deck-btn:hover {{
    background: alpha({PALETTE['purple']}, 0.18);
    box-shadow: 0 0 14px alpha({PALETTE['purple']}, 0.5);
}}
.deck-time {{
    font-family: "JetBrains Mono", monospace;
    font-size: 9px;
...
.deck-progress trough {{
    background: rgba(255, 255, 255, 0.07);
    border-radius: 3px;
    min-height: 4px;
}}
.deck-progress progress {{
    background: linear-gradient(to right, {PALETTE['pink']},
                {PALETTE['cyan']});
    border-radius: 3px;
    min-height: 4px;
}}
...
.ring-title-{name} {{
    ...
    text-shadow: 0 0 8px alpha({c}, 0.55);
}}
"""
