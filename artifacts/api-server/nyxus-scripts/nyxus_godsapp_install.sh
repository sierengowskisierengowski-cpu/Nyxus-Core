#!/usr/bin/env bash
# ============================================================================
# NYXUS GodsApp · one-shot installer
# Downloads nyxus-godsapp.tgz, extracts, runs install.sh
# © 2026 Joseph Sierengowski — NYX-J5W-2026-SIERENGOWSKI-LOCKED
#
#   curl -fsSL https://nyxus-core.replit.app/api/download/nyxus/nyxus_godsapp_install.sh | bash
# ============================================================================
set -euo pipefail

BASE_URL="${NYXUS_BASE_URL:-https://nyxus-core.replit.app}"
TGZ="nyxus-godsapp.tgz"
WORK="$(mktemp -d /tmp/nyxus-godsapp.XXXXXX)"
...
if [[ $EUID -ne 0 ]]; then
  if command -v sudo >/dev/null 2>&1; then
    ...
    if [[ -f "$0" && -r "$0" ]]; then
      ...
      exec sudo -E bash -c "curl -fsSL '${BASE_URL}/api/download/nyxus/nyxus_godsapp_install.sh' | bash"
    fi
  fi
...
step "install"
if [[ ! -d "${WORK}/godsapp" ]]; then
  fail "tarball missing 'godsapp/' directory"; exit 1
fi
bash "${WORK}/godsapp/install.sh"
ok "godsapp installed"

cat <<EOF

──────────────────────────────────────────────────────────────────────

  ${GOLD}NYXUS GodsApp is ready.${R}

  ${B}launch:${R}    nyxus-godsapp
