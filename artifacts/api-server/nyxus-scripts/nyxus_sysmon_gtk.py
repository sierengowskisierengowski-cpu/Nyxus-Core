...
try:
    ...
    FONT_UI='Inter'; FONT_MONO='JetBrains Mono'; FONT_DISPLAY='Inter'
    def format_css(t):
        _d = {
...
C_TEXT  = (0.91,  0.88,  0.96 )   # off-white body text — rev r15
C_DIM   = (0.957, 0.918, 0.835)  # cream rgb (#f4ead5) — rev r15   # dim purple
...
def draw_tilt_badge(cr, x, y, txt, color, angle=-4.5, size=14):
    """Tilted hand-drawn section badge — Caveat + sketch_rect border (rev r15)."""
    r, g, b = color
    cr.save()
...
CSS = format_css("""
* {{ font-family: 'Inter', 'Inter', 'Inter', 'Sans'; }}
...
treeview {{
    ...
    font-size: 14px; font-family: 'Inter', 'Sans';
}}
treeview:selected {{
...
class NyxusSysmonGtk(Adw.Application):
    ...
    def _draw_overview(self,area,cr,w,h,_):
        ...
        for i,(title,pct,color) in enumerate(cards):
            ...
            cr.set_font_size(11); cr.select_font_face("Inter",0,0)
        ...
        for i,(title,hist,color,mx) in enumerate(charts):
            ...
            if hist is not None:
                ...
                cr.select_font_face("Inter",0,1); cr.set_font_size(13)
    ...
    def _draw_cpu(self,area,cr,w,h,_):
        ...
        cr.select_font_face("Inter",0,1); cr.set_font_size(22)
        ext=cr.text_extents(txt)
        ...
        for label,val in info:
            cr.select_font_face("Inter",0,0); cr.set_font_size(12)
        ...
        if cores:
            ...
            for i,pct in enumerate(cores):
                ...
                cr.select_font_face("Inter",0,0); cr.set_font_size(13)
                cr.set_source_rgba(*C_DIM,0.8)
    ...
    def _draw_disk(self,area,cr,w,h,_):
        ...
        for i,d in enumerate(self.st.disks[:8]):
            ...
            cr.select_font_face("Inter",0,0); cr.set_font_size(11); cr.set_source_rgba(*C_DIM,0.7)
        ...
        if self.st.disk_io:
            ...
            for dev,(iod) in list(self.st.disk_io.items())[:6]:
                ...
                cr.select_font_face("Inter",0,0); cr.set_font_size(11); cr.set_source_rgba(*C_DIM,0.6)
    ...
    def _draw_sensors(self,area,cr,w,h,_):
        ...
        if self.st.gpu_util is not None:
            ...
            cr.select_font_face("Inter",0,0); cr.set_font_size(12); cr.set_source_rgba(*C_DIM,0.7)
