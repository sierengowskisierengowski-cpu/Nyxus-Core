...
if ! command -v eww >/dev/null 2>&1; then
  ...
  if command -v paru >/dev/null 2>&1; then
    ...
    # Build from the AUR PKGBUILD — it carries the patch needed for current
    # Arch glib2 (raw `cargo install eww` fails on dbusmenu-glib).
    sudo pacman -S --noconfirm --needed git rust cargo pkgconf base-devel \
