...
if [[ $EUID -ne 0 ]]; then
  if command -v sudo >/dev/null 2>&1; then
    ...
    if [[ -f "$0" && -r "$0" ]]; then
      ...
      exec sudo -E bash -c "curl -fsSL 'https://nyxus-core.replit.app/api/download/nyxus/nyxus_notepad_install.sh' | bash"
    fi
  fi
...
TGZ_URL="${BASE}/nyxus-notepad.tgz"

step "fetch nyxus-notepad.tgz"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
if [[ -n "${NYXUS_OFFLINE_DIR:-}" && -f "${NYXUS_OFFLINE_DIR}/nyxus-notepad.tgz" ]]; then
  cp -f "${NYXUS_OFFLINE_DIR}/nyxus-notepad.tgz" "$TMP/nyxus-notepad.tgz"
elif command -v curl >/dev/null; then
  curl -fsSL "$TGZ_URL" -o "$TMP/nyxus-notepad.tgz" || { fail "download failed"; exit 1; }
elif command -v wget >/dev/null; then
  wget -q -O "$TMP/nyxus-notepad.tgz" "$TGZ_URL" || { fail "download failed"; exit 1; }
else
  fail "neither curl nor wget present"; exit 1
fi
ok "downloaded $(du -h "$TMP/nyxus-notepad.tgz" | cut -f1)"

step "extract"
tar xzf "$TMP/nyxus-notepad.tgz" -C "$TMP"
ok "extracted"

step "run install.sh"
chmod +x "$TMP/nyxus-notepad/install.sh"
"$TMP/nyxus-notepad/install.sh" "$@"
