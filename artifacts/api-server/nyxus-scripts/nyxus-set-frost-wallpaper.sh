...
MODE="${1:-fill}"   # fill | center | tile  (default fill)
...
case "$DAEMON" in
  swaybg)
    ...
    sleep 0.3
    nohup swaybg -i "$WP_DST" -m "$MODE" >/dev/null 2>&1 &
    disown 2>/dev/null || true
    ok "swaybg restarted with ${WP_NAME} (mode: ${MODE})"
    ;;
  hyprpaper)
...
echo "    nyxus-set-frost-wallpaper.sh fill     # stretch to fill (default)"
echo "    nyxus-set-frost-wallpaper.sh center   # sharp small image, dark borders"
