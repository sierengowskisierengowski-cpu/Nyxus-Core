...
class WallpaperStudio(Adw.Application):
    ...
    def _build_window(self) -> None:
        ...
        css.load_from_data(
            ...
            .nyx-tile.selected { border: 2px solid #7d3dff; }
    ...
    def _build_adjustments_page(self) -> Gtk.Widget:
        ...
        if hasattr(Gtk, "ColorButton"):
            ...
            rgba = Gdk.RGBA()
            rgba.parse(self.cfg.get("tint_color", "#7d3dff"))
            color_btn.set_rgba(rgba)
            color_btn.connect("color-set", self._on_tint_color)
            box.append(self._labeled_row("Tint Color", color_btn))
        else:
            tint_entry = Gtk.Entry()
            tint_entry.set_text(self.cfg.get("tint_color", "#7d3dff"))
            tint_entry.set_placeholder_text("#7d3dff")
        ...
        key = json.dumps(
            {
                ...
                "tc": self.cfg.get("tint_color", "#7d3dff"),
    ...
    def _apply_adjustments(self, src: Path) -> Path:
        ...
        if self.cfg.get("tint_enabled", False):
            tint = self.cfg.get("tint_color", "#7d3dff").lstrip("#")
            if len(tint) == 6:
                r = int(tint[0:2], 16)
                g = int(tint[2:4], 16)
                b = int(tint[4:6], 16)
