...
# ──────────────────────────────────────────────────────────────────────
# hypridle contract — shared by Settings ▸ Power and Settings ▸ Screensaver.
# Both pages must write the SAME file the SAME way, or one silently undoes
# the other (that is how the screensaver stage got dropped: the Power page's
# generator was the only writer and did not know about it).
# ──────────────────────────────────────────────────────────────────────
HYPRIDLE_CONF_PATH = str(Path.home() / ".config/hypr/hypridle.conf")
HYPRIDLE_DEFAULTS = {
    "saver_on":           1,   # show the NYXUS urban-alien screensaver
    "saver_secs":       300,   # 5 min  — dim + launch screensaver
    "dim_secs":           0,   # standalone dim ONLY when saver is off
...
def hypridle_prefs() -> dict:
    prefs = load_prefs()
    p = prefs.setdefault("power", {})
    for k, v in HYPRIDLE_DEFAULTS.items():
        p.setdefault(k, v)
    return p


def build_hypridle_conf(p: dict) -> str:
    """Pure NYXUS-prefs → hypridle.conf text (no I/O, so it is unit-testable).

    ★ This emits the WHOLE NYXUS idle pipeline, not just the timers a page
    shows. The previous generator dropped the 45s idle-glass and the
    `nyxus-screensaver` launch entirely, so the first idle-timer change in
    Settings silently stopped the urban-alien screensaver from ever
    appearing. The stages mirror the shipped hypridle.conf; the timers are
    the only thing the user changes.
    """
    saver_on = int(p.get("saver_on", 1)) and have("nyxus-screensaver")
    idle_glass = str(Path.home() / ".config/hypr/scripts/nyxus-idle-glass.sh")
    has_glass = Path(idle_glass).exists()
    lock_cmd = ("nyxus-sound lock ; pidof hyprlock || hyprlock"
                if have("nyxus-sound") else "pidof hyprlock || hyprlock")
    lines = [
        "# nyxus-managed: regenerated by Settings → Power / Screensaver. "
        "Manual edits will be overwritten.",
        "general {",
        f"    lock_cmd = {lock_cmd}",
        *(["    unlock_cmd = nyxus-sound unlock"] if have("nyxus-sound") else []),
    ...
    if saver_on and has_glass:
        lines += [
            "listener {",
            "    timeout    = 45",
            f"    on-timeout = {idle_glass} on",
            f"    on-resume  = {idle_glass} off",
            "}",
            "",
        ]
    if saver_on and p["saver_secs"] > 0:
        _resume = "brightnessctl -r" if have("brightnessctl") else "true"
        if has_glass:
            _resume += f" ; {idle_glass} off"
        _resume += " ; pkill -f nyxus_screensaver ; eww close screensaver"
        _dim = "brightnessctl -s set 10% ; " if have("brightnessctl") else ""
        lines += [
            "listener {",
            f"    timeout    = {p['saver_secs']}",
            f"    on-timeout = {_dim}nyxus-screensaver &",
            f"    on-resume  = {_resume}",
            "}",
            "",
        ]
    elif p["dim_secs"] > 0 and have("brightnessctl"):
        lines += [
            "listener {",
    ...
    if p["lock_secs"] > 0:
        _lock = ("pkill -f nyxus_screensaver ; loginctl lock-session"
                 if saver_on else "loginctl lock-session")
        lines += [
            "listener {",
...
def regenerate_hypridle() -> bool:
    """Write hypridle.conf from current prefs and restart the service."""
    try:
        cfg = Path(HYPRIDLE_CONF_PATH)
        cfg.parent.mkdir(parents=True, exist_ok=True)
        cfg.write_text(build_hypridle_conf(hypridle_prefs()))
...
class PowerPage(SectionPage):
    ...
    def _hypridle_prefs(self) -> dict:
        return hypridle_prefs()

    def _render_sleep(self) -> None:
        _clear_group(self.sleep_grp)
        p = self._hypridle_prefs()

        # Screensaver on/off drives whether the idle-glass + urban-alien saver
        # stages are emitted at all. When off, "Dim screen after" becomes the
        # visible idle behaviour; when on, the saver stage does the dimming.
        saver_row = Adw.SwitchRow(
            title="Show the NYXUS screensaver when idle",
            subtitle="urban-alien saver over a dimmed screen "
                     "(off = just dim the screen)")
        saver_row.set_active(bool(int(p.get("saver_on", 1))))
        saver_row.connect("notify::active", self._on_saver_toggle)
        self.sleep_grp.add(saver_row)

        timers = [("saver_secs", "Show screensaver after",
                   "dim + the NYXUS screensaver")] if int(p.get("saver_on", 1)) \
            else [("dim_secs", "Dim screen after",
        ...
        for key, title, sub in timers:
            cur = int(p.get(key, 0))
            self.sleep_grp.add(self._duration_row(title, sub, cur, key))

    def _on_saver_toggle(self, row: Adw.SwitchRow, _ps) -> None:
        prefs = load_prefs()
        prefs.setdefault("power", {})["saver_on"] = 1 if row.get_active() else 0
        save_prefs(prefs)
        self._regenerate_hypridle()
        self._render_sleep()   # swap the saver-timer row for the dim-timer row
    ...
    def _regenerate_hypridle(self) -> None:
        """Rewrite hypridle.conf from prefs. Delegates to the shared writer so
        Settings ▸ Screensaver and Settings ▸ Power can never disagree."""
        if not regenerate_hypridle():
...
class IdleSaverPage(SectionPage):
    ...
    # Timing choices offered for "Show screensaver after".
    _SAVER_CHOICES = [(60, "1 minute"), (120, "2 minutes"),
    ...
    def build(self) -> None:
        ...
        if have("nyxus-screensaver"):
            # The real control: on/off + when. Writes the shared hypridle prefs
            # and regenerates the conf through the same writer Power uses, so
            # the two pages can never disagree.
            self._saver = Adw.SwitchRow(
                title="Show the NYXUS screensaver when idle",
                subtitle="urban-alien saver over a dimmed screen")
            self._saver.set_active(bool(int(p.get("saver_on", 1))))
            self._saver.connect("notify::active", self._on_toggle)
            g.add(self._saver)

            self._after = Adw.ComboRow(
                title="Show screensaver after",
                subtitle="idle time before it appears")
            mdl = Gtk.StringList()
            for _, lbl in self._SAVER_CHOICES:
                mdl.append(lbl)
            self._after.set_model(mdl)
            cur = int(p.get("saver_secs", 300))
            idx = next((i for i, (s, _) in enumerate(self._SAVER_CHOICES)
                        if s == cur), 2)
            self._after.set_selected(idx)
            self._after.set_sensitive(bool(int(p.get("saver_on", 1))))
            self._after.connect("notify::selected", self._on_after)
            g.add(self._after)
    ...
    def _on_toggle(self, row: Adw.SwitchRow, _ps) -> None:
        prefs = load_prefs()
        prefs.setdefault("power", {})["saver_on"] = 1 if row.get_active() else 0
        save_prefs(prefs)
        if regenerate_hypridle():
            self._after.set_sensitive(row.get_active())
            self.toast("screensaver " + ("on" if row.get_active() else "off"))
        else:
            self.toast("could not update hypridle")

    def _on_after(self, row: Adw.ComboRow, _ps) -> None:
        secs = self._SAVER_CHOICES[row.get_selected()][0]
        prefs = load_prefs()
        prefs.setdefault("power", {})["saver_secs"] = secs
        save_prefs(prefs)
        self.toast("screensaver delay saved"
                   if regenerate_hypridle() else "could not update hypridle")
