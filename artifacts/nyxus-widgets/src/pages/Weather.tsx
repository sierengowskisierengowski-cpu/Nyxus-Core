    ...
    switch (condition) {
      ...
      case 'CLEAR_NIGHT': return (
        <div className="absolute inset-0 overflow-hidden bg-gradient-to-b from-[#04021a] via-[#08080e] to-[#08080e]">
          ...
          <div className="absolute top-10 right-16 text-blue-300/60 drop-shadow-[0_0_30px_rgba(43,210,255,0.4)]">
...
export default function Weather() {
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-[#08080e]">
        <div className="flex flex-col items-center gap-5">
          <div className="h-14 w-14 border-4 border-[#ff2dad] border-t-transparent rounded-full animate-spin" style={{ boxShadow: '0 0 20px #ff2dad88' }} />
          <span style={{ fontFamily: "'Caveat',cursive", fontSize: 28, color: '#ff2dad', textShadow: '0 0 12px #ff2dad' }}>
            LOADING WEATHER...
          </span>
        </div>
      </div>
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-[#08080e] text-white">
        <div style={{
          ...
          border: '2px solid #7d3dff', borderRadius: 16, padding: 32,
          boxShadow: '0 0 40px #7d3dff44',
        }}>
          <h2 style={{ fontFamily: "'Caveat',cursive", fontSize: 36, color: '#7d3dff', textShadow: '0 0 12px #7d3dff', marginBottom: 8, textAlign: 'center' }}>
            LOCATION
          </h2>
          ...
          <form onSubmit={handleCitySearch} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Input
              ...
              style={{ background: '#030206', border: '1px solid #2bd2ff', color: '#eef2fa', fontFamily: "'JetBrains Mono',monospace", textAlign: 'center', letterSpacing: '0.2em', boxShadow: '0 0 10px #2bd2ff44' }}
            />
            <Button type="submit" disabled={loading} style={{ background: '#ff2dad', color: '#000', fontFamily: "'Caveat',cursive", fontSize: 20, fontWeight: 700, letterSpacing: '0.1em', boxShadow: '0 0 14px #ff2dad66' }}>
            ...
            {error && <p style={{ color: '#ff8a1e', fontSize: 12, textAlign: 'center', fontFamily: "'JetBrains Mono',monospace" }}>{error}</p>}
          </form>
        </div>
      </div>
    );
  ...
  const color     = conditionColor[condition] ?? '#ff2dad';
      ...
      <div className="absolute inset-0 flex" style={{ padding: '32px 40px' }}>
        ...
        <div style={{ width: '42%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ padding: '16px 18px', background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(255,138,30,0.35)', borderRadius: 12, backdropFilter: 'blur(14px) saturate(1.6)' }}>
              ...
              <span style={{ fontFamily: "'Caveat',cursive", fontSize: 30, fontWeight: 700, color: '#ff8a1e', textShadow: '0 0 10px #ff8a1e88' }}>
                {wind} <span style={{ fontSize: 16, fontWeight: 500 }}>MPH</span>
              </span>
            </div>
            <div style={{ padding: '16px 18px', background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(43,210,255,0.35)', borderRadius: 12, backdropFilter: 'blur(14px) saturate(1.6)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              ...
              <span style={{ fontFamily: "'Caveat',cursive", fontSize: 30, fontWeight: 700, color: '#2bd2ff', textShadow: '0 0 10px #2bd2ff88' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingLeft: 40 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
            {daily.time.slice(0, 5).map((date: string, i: number) => {
              ...
              const dc_col  = conditionColor[dc] ?? '#ff2dad';
              const isToday = i === 0;
              ...
              return (
                <div key={date} style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    <span style={{ fontFamily: "'Caveat',cursive", fontSize: 22, fontWeight: 700, color: '#ff2dad', textShadow: '0 0 8px #ff2dad66' }}>
                      {Math.round(daily.temperature_2m_max[i])}°
                    </span>
                    <span style={{ fontFamily: "'Caveat',cursive", fontSize: 17, color: '#2bd2ff' }}>
                      {Math.round(daily.temperature_2m_min[i])}°
                    </span>
                  </div>
                  {precip > 0 && (
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 8, color: '#2bd2ff', letterSpacing: '0.1em' }}>
                      ▼ {precip}%
                    </span>
                  )}
                </div>
          ...
          {/* Precipitation bar */}
          {precip0 > 0 && (
            <div style={{ marginTop: 20, padding: '16px 20px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(43,210,255,0.2)', borderRadius: 12, backdropFilter: 'blur(14px) saturate(1.6)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                ...
                <span style={{ fontFamily: "'Caveat',cursive", fontSize: 18, fontWeight: 700, color: '#2bd2ff' }}>{precip0}%</span>
              </div>
              <div style={{ height: 5, background: 'rgba(255,255,255,0.08)', borderRadius: 99, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${precip0}%`, background: 'linear-gradient(90deg, #2bd2ff, #7d3dff)', borderRadius: 99, transition: 'width 1s ease' }} />
              </div>
            </div>
          )}
          ...
          <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div style={{ padding: '12px 16px', background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(125,61,255,0.2)', borderRadius: 12, backdropFilter: 'blur(14px) saturate(1.6)' }}>
              ...
              <span style={{ fontFamily: "'Caveat',cursive", fontSize: 24, fontWeight: 700, color: '#7d3dff', textShadow: '0 0 8px #7d3dff66' }}>
