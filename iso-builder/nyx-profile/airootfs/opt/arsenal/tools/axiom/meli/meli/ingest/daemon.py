...
from meli.enrichment.geoip_scheduler import GeoIPUpdateScheduler
...
class IngestDaemon:
    ...
    def __init__(self) -> None:
        ...
        self._geoip_scheduler: GeoIPUpdateScheduler | None = None
    ...
    def run(self) -> None:
        ...
        # Start GeoLite2 auto-update scheduler (weekly by default)
        try:
            self._geoip_scheduler = GeoIPUpdateScheduler()
            self._geoip_scheduler.start()
        except Exception as e:
            log.error("GeoLite2 scheduler failed to start", error=str(e))
    ...
    def _shutdown(self) -> None:
        ...
        if self._geoip_scheduler:
            self._geoip_scheduler.stop()
        if self._mqtt:
            self._mqtt.stop()
