...
from meli.utils.helpers import format_honeypot_services as _format_services
...
class AttackersView(Gtk.Box):
    ...
    def _load_data(self) -> None:
        try:
            ...
            from meli.database.models import Attacker, Event
            from sqlalchemy import select, func
            with get_db() as db:
                attackers = db.execute(
                    select(Attacker).order_by(Attacker.total_events.desc()).limit(200)
                ).scalars().all()
                ips = [a.ip for a in attackers]
                # One query for (ip, service) pairs so we can show what each
                # attacker actually probed (SSH? HTTP? both?) without N+1ing.
                services_by_ip: dict[str, list[str]] = {}
                if ips:
                    rows = db.execute(
                        select(Event.source_ip, Event.honeypot_service,
                               func.count(Event.id))
                        .where(Event.source_ip.in_(ips))
                        .group_by(Event.source_ip, Event.honeypot_service)
                    ).all()
                    for ip, svc, _cnt in rows:
                        services_by_ip.setdefault(ip, []).append(svc or "unknown")
                data = [
                    (a.ip, a.total_events, a.max_severity, a.country_code,
                     ...
                     sorted(set(services_by_ip.get(a.ip, []))))
                    for a in attackers
                ]
    ...
    def _populate_list(self, data: list) -> bool:
        ...
        for (ip, total, max_sev, country, asn, org, first_seen, last_seen,
             is_tor, is_vpn, rep, services) in data:
            row = Gtk.ListBoxRow()
            row._ip = ip
            row._data = (ip, total, max_sev, country, asn, org, first_seen,
                         last_seen, is_tor, is_vpn, rep, services)
            ...
            # Tiny per-service tag so SSH-only vs HTTP-only vs both is
            # visible at a glance in the list (not just the detail panel).
            svc_lbl = Gtk.Label(label=_format_services(services))
            svc_lbl.set_size_request(110, -1)
            svc_lbl.set_xalign(0)
            svc_lbl.add_css_class("dim-label")
            ...
            for w in [flag_lbl, ip_lbl, sev_lbl, svc_lbl, cnt_lbl]:
                box.append(w)
            row.set_child(box)
    ...
    def _show_detail(self, data: tuple) -> None:
        (ip, total, max_sev, country, asn, org, first_seen, last_seen,
         is_tor, is_vpn, rep, services) = data
        ...
        for label, value in [
            ...
            ("Honeypots Hit", _format_services(services) if services else "—"),
