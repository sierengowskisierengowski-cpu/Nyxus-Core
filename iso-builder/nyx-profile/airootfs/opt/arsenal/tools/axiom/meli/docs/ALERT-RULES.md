...
## Built-in Alert Rules (seeded on first start)

In addition to severity classification rules in `meli/classification/default_rules.yaml`, Meli seeds two **alert** rules into the `alert_rules` table on first startup. These fire notifications (not just classify severity):

| Rule | Trigger | Default channels |
|------|---------|------------------|
| New attacker IP (first seen / silent 7+ days) | `attacker_is_new_7d == True` | `desktop`, `webhook` |
| Login brute-force burst (50+ attempts / minute) | `action_type == login_attempt` AND `attacker_login_attempts_1min >= 50` | `desktop`, `webhook` |

Seeding is idempotent: rules are inserted only if a row with the same name doesn't already exist. Subsequent runs preserve any user edits (enabled flag, channels, cooldown). Both rules appear in **Alert Rules → Rules** where they can be enabled, disabled, or deleted like any other rule.
...
| `attacker_is_new_7d` | Alert engine | `True` if IP unseen in last 7 days |
| `attacker_login_attempts_1min` | Alert engine | Login attempts from IP in last 60s |
...
| `ntfy` | `ntfy_url`, optional `ntfy_token` | POST to ntfy.sh topic URL |
