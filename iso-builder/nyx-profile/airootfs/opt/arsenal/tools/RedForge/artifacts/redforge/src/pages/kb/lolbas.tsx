...
import { useListLolbas } from "@workspace/api-client-react";
...
export default function LolbasList() {
  useDocumentTitle("LOLBAS");
  const [search, setSearch] = useState("");
  const { data: items, isLoading } = useListLolbas({ search: search || undefined });
    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold tracking-widest text-primary uppercase">LOLBAS</h2>
        </div>
      </div>
      ...
      <Card className="flex-1 overflow-hidden flex flex-col">
        <div className="flex-1 overflow-auto">
          {isLoading ? (
            <div className="p-8 text-center text-muted-foreground font-mono animate-pulse">QUERYING LOLBAS INDEX...</div>
            ...
            <Table>
              <TableHeader className="bg-muted/50 sticky top-0">
                <TableRow>
                  ...
                  <TableHead className="font-mono text-[10px] tracking-widest">TAGS</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {items.map(item => (
                  <TableRow key={item.name} className="hover:bg-muted/30">
                    <TableCell className="font-mono font-bold text-primary">
                      <Link href={`/kb/lolbas/${item.name}`} className="hover:underline">{item.name}</Link>
                    </TableCell>
