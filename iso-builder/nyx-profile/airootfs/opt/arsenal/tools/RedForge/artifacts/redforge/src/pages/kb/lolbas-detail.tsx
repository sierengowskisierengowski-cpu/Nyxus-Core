import { useGetLolbasEntry } from "@workspace/api-client-react";
...
export default function LolbasDetail() {
  const { name } = useParams();
  useDocumentTitle(`LOLBAS: ${name}`);
  const { data: item, isLoading } = useGetLolbasEntry(name!);
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <Link href="/kb/lolbas" className="text-xs text-muted-foreground hover:text-primary flex items-center gap-2 mb-4 w-fit">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold font-mono text-green-500">{item.name}</h1>
        </div>
        <p className="text-muted-foreground text-lg">{item.summary}</p>
        <div className="flex flex-wrap gap-2 mt-4">
          {item.tags?.map(t => (
            <Badge key={t} variant="secondary" className="font-mono text-[10px] tracking-widest">{t}</Badge>
          ))}
        </div>
      </div>
      ...
      {item.examples && item.examples.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-bold uppercase tracking-widest border-b border-border pb-2">Execution Examples</h3>
