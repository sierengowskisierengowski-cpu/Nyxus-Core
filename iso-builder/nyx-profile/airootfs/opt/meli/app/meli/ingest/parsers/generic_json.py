...
class GenericJsonParser:
    ...
    def _parse_canonical(self, raw: dict) -> dict[str, Any] | None:
        ...
        return {
            ...
            # Cowrie file-download events carry the sample hash as `shasum`;
            # other sources use `sha256`. Accept either so the payloads
            # aggregation table populates for real captured samples.
            "payload_hash": details.get("sha256") or details.get("shasum"),
            "payload_url": details.get("url"),
            "payload_filename": details.get("filename") or details.get("outfile"),
        }
