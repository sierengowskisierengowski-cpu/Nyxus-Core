...
def setup_logging(debug: bool = False) -> None:
    ...
    # (e.g. `meli --status` / `meli --report` JSON consumed by the Bifrost
    # UI). The ingest daemon's systemd unit redirects both streams to its
    # log file, so this doesn't change daemon logging.
    logging.basicConfig(
        format="%(message)s",
        stream=sys.stderr,
        level=level,
