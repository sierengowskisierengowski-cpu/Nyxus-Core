...
| App Store                       | PASS (built to contract 2026-05-12) | New `nyxus_store.py` — Settings-style `Adw.NavigationSplitView` with sidebar sections **Featured / Installed / Updates / Search / Repositories / About**. Curated 12-app featured list with one-click install (auto-marks "Installed" for already-present packages). Installed = `pacman -Qe` with Remove. Updates = `checkupdates` (or `pacman -Qu` fallback) + `paru/yay -Qua` + `flatpak remote-ls --updates`, with Update-All bar per backend. Search = debounced 350ms across pacman + AUR + flatpak (capped 60/backend). Privilege model: pacman install/remove/upgrade goes through `pkexec pacman …` (root-required); AUR helpers (paru/yay) deliberately run as the invoking user — `paru/yay` will internally `sudo`/`pkexec` for the pacman portion, since AUR build steps must NOT run as root; flatpak runs at user scope (no pkexec) by default. Every command is launched inside a terminal window (alacritty/kitty/foot/wezterm/xterm fallback chain) so polkit prompts and conflict prompts are visible. Honest §9 empty states everywhere ("Everything is up to date", "No matches", "No AUR helper installed"). Backend matrix in About shows `pacman / checkupdates / paru-yay / flatpak / pkexec` availability. `NYXUS_DRY_RUN=1` for safe testing. |
| Power Menu (standalone)         | PASS (built to contract 2026-05-12) | New `nyxus_powermenu.py` — standalone `Adw.ApplicationWindow` replacing the legacy `nyxus_powermenu.py` slot that the EWW overlay had stolen. 6 tactile tiles in a 3×2 grid: Lock (`hyprlock` → `loginctl lock-session` fallback), Suspend (`systemctl suspend`), Logout (`hyprctl dispatch exit`), Restart, Shutdown, Cancel. Restart/Shutdown/Logout gate behind `Adw.MessageDialog` confirm with destructive styling. Esc dismisses. `NYXUS_DRY_RUN=1` honored. EWW powermenu overlay (Super+Esc) stays — the standalone app is for menu/launcher invocation. |
| App family Adw consistency      | PASS (audited 2026-05-12) | All 13 GUI Python apps in `nyxus-scripts/` — notepad, stickies, notes, sysmon_gtk, settings, control, terminal, launcher, screenshot, **store**, **powermenu**, screensaver, welcome, fog, demon_wake — subclass `Adw.Application`, call `Adw.init()`, and force `Adw.ColorScheme.FORCE_DARK` on activate. Same shared `nyxus_chrome` graffiti background + `nyxus_palette` tokens. Doctor stays CLI by design. `APPS_LIST` in `iso-builder/build-iso.sh` cleaned: phantom `weather`/`quicksettings` entries removed (those live in EWW), duplicate `settings:` collapsed, `notes` + `store` + `powermenu` entries added. Every entry maps to a real `nyxus_<mod>.py`. The Download Portal at `nyxus-web` exposes the full inventory at hash route `#/build` so the build can be inspected without booting the ISO. |
